Notes on running simulation:
 ...
 - command line:
   ...
   #+END_SRC
 - now both joint positions and force torque sensor reading should
   be available!
 - remember to load parameters with
   #+BEGIN_SRC sh
     roscd infinite_interaction/config && rosparam load admittance_params.yaml && rosparam list
   #+END_SRC
